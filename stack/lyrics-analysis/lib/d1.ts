...
/** Run multiple SQL statements sequentially via d1Fetch. */
export async function batchWriteD1(
  statements: Array<{ sql: string; params?: (string | number | null)[] }>,
): Promise<void> {
  for (const stmt of statements) {
    const resp = await d1Fetch({ sql: stmt.sql, params: stmt.params ?? [] });
    if (!resp.success || !resp.result?.[0]?.success) {
      throw new Error(`D1 batch write failed: ${JSON.stringify(resp.errors)}`);
    }
  }
}
